...
problem, **energy-unstable**. Here is the intuition. Multiply the semi-discrete scheme by
the solution and sum over the mesh — this measures how the total "energy"
\\( \tfrac12\int u^2 \\) (or, for systems, a convex *entropy*) evolves in time. The volume
terms telescope cleanly. The surface terms, with a central flux, leave behind a residual
that has **no definite sign**: depending on the jump \\( [u]=u^--u^+ \\) and the sign of the
local wave speed, the discrete energy can *grow*. There is nothing in the central flux to
remove energy from the under-resolved modes at the interface, so any small ripple at the
...
evaluated between every pair of nodes on a line:

\\[
(\nabla\cdot\mathbf{F})_i \;\approx\; \frac{2}{J}\sum_{j} D_{ij}\,
...
- **Burgers** uses the entropy-conserving flux \\( (u_L^2 + u_L u_R + u_R^2)/6 \\).
- **Incompressible convection** uses the **kinetic-energy-preserving (KEP)** flux,
  the product of arithmetic means \\( \{u_j\}\{u_i\} \\) — exactly what keeps the discrete
  kinetic energy of the convection term in check at high Re.
- **Euler** uses the **Chandrashekar (2013)** entropy-conserving flux, which needs the
  numerically-careful **logarithmic mean** \\( (a-b)/(\ln a - \ln b) \\) (gale's `ln_mean`,
  with a series expansion near \\( a=b \\) to avoid catastrophic cancellation).
