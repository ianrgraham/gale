...
the result is deterministic (the summation order per output node is fixed)
regardless of how the blocks happen to be scheduled.
...
So in `gale-gpu` the kernels carry `2d`/`3d`/`mg` suffixes (`advect2d_rhs`,
`advect3d_rhs`, `advect2d_mg_rhs`) and shared linear-algebra primitives are defined
once, crate-wide — hence `gale-gpu` is one crate with many operator modules under a
single bundle, rather than a swarm of tiny per-operator crates.
...
result is bit-for-bit the single-GPU advection result, split — a pure gather of face
traces does its per-node summations in the same order as the monolithic operator.
gale runs **real two-device advection in both 2D and 3D** on its 2× Titan V box
today.
